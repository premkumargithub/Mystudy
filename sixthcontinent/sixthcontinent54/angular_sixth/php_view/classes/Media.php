...
class Media {
    ...
    static function mediaYoutube($url) {
        ...
        if (preg_match("/(.*?)v=(.*?)($|&)/i", $url, $matching)) {
            ...
            array_push($media, '<iframe id="' . date("YmdHis") . $vid . '" style="display: none; margin-bottom: 5px;" width="499" height="368" src="https://www.youtube.com/embed/' . $vid . '" frameborder="0" allowfullscreen></iframe>');
        } else {
            array_push($media, "", "");
        }
        return $media;
